...
DEFAULT_DESTINATIONS = ("bigquery", "snowflake")
...
def _apply_incremental_hints(source, tables: Iterable[str], write_disposition: str) -> None:
    if not _incremental_enabled(write_disposition):
        return

    merge_write_disposition = _merge_write_disposition_config()
    for table in tables:
        ...
        if resource is None:
            logger.warning("Could not find dlt resource for table %s; merge hints were not applied", table)
            continue
        if not primary_key or not cursor:
...
def build_source(tables: Iterable[str], write_disposition: str):
    ...
    _apply_incremental_hints(source, selected_tables, write_disposition)
    return source
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=_csv_list(os.environ.get("MIGRATION_DESTINATIONS"), DEFAULT_DESTINATIONS),
