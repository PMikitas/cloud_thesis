...
- BigQuery settings from `data-migration-scripts/.env` plus ADC
...
operational tables refreshed by the migration bridge.
...
The current `metrics_scripts.sql` is BigQuery dialect, so the scheduler defaults
to `METRICS_WAREHOUSES=bigquery`. If you later add Redshift or Snowflake query
variants, switch `METRICS_WAREHOUSES` or pass `--warehouses` explicitly.
