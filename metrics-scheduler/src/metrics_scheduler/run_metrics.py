...
QUALIFIED_NAME_RE = re.compile(r"\bperformance_tracking\.([A-Za-z_][A-Za-z0-9_]*)\b", re.IGNORECASE)
...
BIGQUERY_DATASET = MIGRATION_ENV_VALUES.get("BQ_DATASET", "performance_tracking")
BIGQUERY_EVENT_TABLE = "api_performance"
...
SNOWFLAKE_DEFAULT_SCHEMA = (
    MIGRATION_ENV_VALUES.get("SF_SCHEMA")
    or MIGRATION_ENV_VALUES.get("SNOWFLAKE_SCHEMA")
    or SNOWFLAKE_PROPERTY_VALUES.get("snowflake.schema", "performance_tracking")
).strip().lower()
SNOWFLAKE_EVENT_TABLE = SNOWFLAKE_PROPERTY_VALUES.get("snowflake.table", "API_TRACKING").lower()
SNOWFLAKE_EVENT_TABLE_OVERRIDE = os.environ.get("METRICS_SNOWFLAKE_EVENT_TABLE", "").strip().lower() or None
...
def rewrite_relation_names(sql: str, warehouse: str) -> str:
    def repl(match: re.Match[str]) -> str:
        ...
        if warehouse == "snowflake":
            if table_name.lower() == "api_performance":
...
def execute_query_for_warehouse(warehouse: str, sql: str) -> QueryExecutionResult:
    if warehouse == "redshift":
        return fetch_redshift_dataframe(sql)
    if warehouse == "bigquery":
        return fetch_bigquery_dataframe(sql)
    if warehouse == "snowflake":
        resolved_table = resolve_snowflake_event_table()
        result = fetch_snowflake_dataframe(apply_snowflake_event_table(sql, resolved_table))
        return QueryExecutionResult(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    default_warehouses = [
        warehouse.strip()
        for warehouse in os.environ.get("METRICS_WAREHOUSES", "bigquery").split(",")
        if warehouse.strip()
    ]
