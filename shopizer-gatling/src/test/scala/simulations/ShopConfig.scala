...
object ShopConfig {
  ...
  def initTracingHeaders = exec { session =>
    val clientId = session("clientHeader").asOption[String]
      .getOrElse("client-" + UUID.randomUUID().toString)
    val sessionId = session("sessionHeader").asOption[String]
      .getOrElse("session-" + UUID.randomUUID().toString)

    session
      .set("clientHeader", clientId)
  ...
  val httpProtocol = http
    ...
    .header("X-Client-Id", "#{clientHeader}")
    .header("X-Session-Id", "#{sessionHeader}")
    .disableWarmUp
}
