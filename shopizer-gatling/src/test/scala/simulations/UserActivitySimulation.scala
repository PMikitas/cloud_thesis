 ...
 *     is triggered.
 *
 * Each virtual user carries a stable `X-Session-Id` header so the server-side
 * BigQuery tracing filter can group events into sessions and the following
...
class UserActivitySimulation extends Simulation {
  ...
  private val addToCartStep =
    exec(ensureAuthed)
      .doIf(session => session("productSkus").asOption[Seq[String]].exists(_.nonEmpty)) {
  ...
  private val initSessionHeader = exec { session =>
    session.set("sessionHeader", java.util.UUID.randomUUID().toString)
  }

  // ── Scenarios ─────────────────────────────────────────────────────────────

  private val registeredUserScn = scenario("Registered User Activity")
    .exec(initSessionHeader)
    .feed(userFeeder)
    .exec(registerCustomer).pause(1, 2)
    .exec(loginCustomer).pause(1, 2)
    .exec(sessionLoop)
  ...
  private val activityProtocol = httpProtocol.header("X-Session-Id", "#{sessionHeader}")
